...
void process_delete(int sock, struct FORWARD_chain *chain, char* buffer)
{
	...
	struct fw_rule * aux_pointer_to_delete;
	struct fw_rule * aux_pointer;
	...
	if(chain->num_rules == 1) {
		...
		if(number == 1) {
			...
			aux_pointer_to_delete = aux_pointer->next_rule;
			aux_pointer->next_rule = aux_pointer_to_delete->next_rule;
			free(aux_pointer_to_delete);
		}
	}
