...
rule get_rule()
{
	...
	char buffer_sd_port[MAX_BUFF_SIZE];
	int mask, port;
	rule new_rule;
	bool correct = FALSE;
	...
	printf("Introdueix la regla seguint el format:\n[src|dst] address netmask [sport|dport|no port=0] [port|no port=0]\n\n");
	scanf("%s %s %d %s %d", buffer_sd_addr, buffer_addr, &mask, buffer_sd_port, &port);
	...
	do{
		if(0 == strcmp(buffer_sd_addr, "src")) {
			...
			if(0 == strcmp(buffer_sd_addr, "dst")) {
				...
				printf("Error input [src|dst]\nTorna a insertar [src|dst]: ");
				scanf("%s", buffer_sd_addr);
			}	
		}
	}while(!correct);
	...
	do{
		if(0 == strcmp(buffer_sd_port, "sport")) {
			new_rule.src_dst_port = htons((unsigned short)SRC_P); 
			correct = TRUE;
		}
		else {
			if(0 == strcmp(buffer_sd_port, "dport")) {
				new_rule.src_dst_port = htons((unsigned short)DST_P); 
				correct = TRUE;
			}
			else {
				if(0 == strcmp(buffer_sd_port, "0")) {
					new_rule.src_dst_port = htons((unsigned short)NO_P); 
					correct = TRUE;
				}
				else {
					printf("Error input [sport|dport]\nTorna a insertar [sport|dport|no port=0]: ");
					scanf("%s", buffer_sd_port);
				}
			}	
		}		
	}while(!correct);
	...
	do{
		if((mask < 0) || (mask > 32)) {
			printf("Error input mask\nTorna a insertar mask: ");
			scanf("%d", &mask);
		}
		else
			correct = TRUE;
	...
	for(i = 0; i < number; i++) {
		...
		if(sd_port == SRC_P)
			...
			if(sd_port == DST_P)
				...
				strcpy(buffer_sd_port, "0");
		}

		if((port == 0) && (sd_port == 0))
			printf("%d- %s %s/%hu\n", i+1, buffer_sd_addr, buffer_addr, mask);
		else
