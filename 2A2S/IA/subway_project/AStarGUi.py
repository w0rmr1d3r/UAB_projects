...
class _Astargui:
    ...
    def __init__(self, master):
        ...
        self.filenameMetro.set("Stations_Final.txt")
        self.filenameConnections.set("Connections_Final.txt")
        self.filenameTimeStations.set("CompletLyon_Final_Time.txt")
        self.filenameInfoVelocity.set("CompletLyon_Final_InfoVelocity.txt")
        self.filenameInfoTransfers.set("CompletLyon_Final_InfoTransfers.txt")
        ...
        self.Label_10.place(x=306./1300*RWidth, y=450./900*RHeight, width=120, height=27)
        ...
        self.city=CityInfo(len(infoVelocity),infoVelocity,infoTransfers,self.connections)
        ...
        if self.id_origen != -1:  # If an origin is selected, continue
            if self.id_desti != -1:  # If a destination is selected, continue
                if self.typePreference != -1:  # If a preference is selected, run ASTAR algorithm and show the Optimal Path
                    time, distance, transfers, stopStations, expanded_nodes, num_depth, visited_nodes, idoptimalpath, min_distance_origin, min_distance_destination = AstarAlgorithm(self.stationList, self.coord_origin, self.coord_destination, int(self.typePreference), self.city, self.flag_redundant)
                    self.Update_Resultant_Path(time, distance, transfers, stopStations, expanded_nodes, visited_nodes, idoptimalpath, min_distance_origin, min_distance_destination, self.coord_origin, self.coord_destination, num_depth)
                else:
    ...
    def Update_Resultant_Path(self, time, distance, transfers, stopStations, expanded_nodes, visited_nodes, idoptimalpath,
        ...
        if time != []:
            ...
            self.Route_Text.insert(END, Print_path(idoptimalpath, self.stationList, min_distance_origin, min_distance_destination, coord_origin, coord_destinationnation))
        else:
...
def Print_path(idoptimalpath, stationList, min_distance_origin, min_distance_destination, coord_origin, coord_destinationnation):
    ...
    if (min_distance_origin != 0):
        stringList = stringList + " WALK FROM: \t[" + str(coord_origin[0]) + "," + str(coord_origin[1]) + "] \t TO : \t" + str(stationList[idoptimalpath[0] - 1].name) + "\n"

    stringList = stringList + "===========================================================\n"
    stringList = stringList + "     ORIGEN :\t" + str(idoptimalpath[0]) + "\t" + str(stationList[idoptimalpath[0] - 1].line) + "\t" + str(stationList[idoptimalpath[0] - 1].name) + "\n"
    stringList = stringList + "===========================================================\n"
